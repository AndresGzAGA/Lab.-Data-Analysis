...
def model(t, k):
    return (To-Tm)*np.power(m.e,-k*(t-to))+Tm

def err_model(t, k, dt, dk):
    return np.sqrt(np.power(m.e,-2*k*(t-to))*dTo**2+(1-np.power(m.e,-k*(t-to)))**2*dTm**2+(t-to)**2*(To-Tm)**2*np.power(m.e,-2*k*(t-to))*dk**2+k**2*(To-Tm)**2*np.power(m.e,-2*k*(t-to))*dt**2)
...
def fit(file_name, Po, Po_err, ci=1, cf=1, xerr=2, yerr_type="Measurement", yerr=3):
    ...
    plt.fill_between(xdataE,model(xdataE, *popt)-err_model(xdataE,popt[0],0.016,pcov[0][0]),model(xdataE, *popt)+err_model(xdataE,popt[0],0.016,pcov[0][0]),alpha=0.5,edgecolor='#CC4F1B', facecolor='#FF9848')
    ...
    plt.legend()
    plt.title('Enfriamiento ajustado de 150 ml de agua en envase de plastico al aire')
    plt.show()
