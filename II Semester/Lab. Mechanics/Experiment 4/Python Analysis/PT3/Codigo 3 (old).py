...
def err_a(o, do):
    return np.sqrt(9.78**2*(np.cos(o))**2*do**2/4) #NOTA: Checar 9.78 en cm o m, o (theta) tiene que estar en radianes!!!
...
def modelR(x, X, Ro, a):
    return (X*Ro)/(X-modelx(x,a))
    
def err_X(o, do, D, dD, r, dr, dP, dRo):
    return np.sqrt(dP**2+(dRo**2+dD**2)*(np.tan(o)**2)+dr**2*(np.sec(o)**2)+do**2*((do-D)*(np.sec(o)**2)+r*np.sec(o)*np.tan(o))**2)
    
def err_modelR(x, dx, a, da, o, do, D, dD, r, dr, P, dP, Ro, dRo):
    X=P+(Ro-D)*np.tan(o)+r/np.cos(o)
    dX=err_X(o, do, D, dD, r, dr, dP, dRo)
    return np.sqrt(X**2/((X-modelx(x,a))**2)*dRo**2+modelx(x,a)**2/((X-modelx(x,a))**4)*dX**2+X**2*Ro**2/((X-modelx(x,a))**4)*err_modelx(x,dx,a,da)**2)
...
def fit(file_name, Po, Po_err, ci=1, cf=1, xerr=2, yerr_type="Measurement", yerr=3):
    ...
    o=np.deg2rad(3)
    do=np.sqrt(3.1415**2/(180**2)*0.5**2)
    ...
    dist_plot(file_name,ci,cf,xerr,yerr_type,yerr) #Plotting just the data
    ...
    plt.fill_between(xdataE,modelR(xdataE, *popt)-err_modelR(xdataE,0.016,popt[0],pcov[0][0],o,do,D,dD,r,dr,P,dP,Ro,dRo),modelR(xdataE, *popt)+err_modelR(xdataE,0.016,popt[0],pcov[0][0],o,do,D,dD,r,dr,P,dP,Ro,dRo),alpha=0.5,edgecolor='#CC4F1B', facecolor='#FF9848')
    #Check if popt[0] corresponds to a, aswell as pcov[0][0] corresponds to da.
    
    plt.plot(xdataE, modelR(xdataE, *Po), 'g-', label='Model')
    plt.fill_between(xdataE, modelR(xdataE,*Po)-err_modelR(xdataE,0.016,Po[2],err_a(do,o),o,do,D,dD,r,dr,P,dP,Ro,dRo), modelR(xdataE, *Po)+err_modelR(xdataE,0.016,Po[2],err_a(do,o),o,do,D,dD,r,dr,P,dP,Ro,dRo),alpha=0.5, edgecolor='#3F7F4C', facecolor='#7EFF99',linewidth=0)

    plt.xlabel('Tiempo [s]')
    plt.ylabel('Distancia [cm]')
    plt.legend()
    plt.title('R(t) con un ángulo de 3 grados desde una vista desde el plano')
    plt.show()
