...
def deleter(file_name,lrow):
    """
    Deletes the rows in the list lrow (list of integers which represents its position).
    The rows should be in increasing order
    """
    D=data_matrix(file_name)
    t=0
    for i in lrow:
        del D[i-t] #Counteracting the fact that the rows shift with the deletion of the last one.
        t+=1
    outfile=open(file_name,"w")
    for i in D:
        j=i[0]
...
def a(file_name):
    """Reads and writes again the same file"""
    D=data_matrix(file_name)
    outfile=open(file_name,"w")
    for row in D:
        column=row[0]
...
#def fix_hor(file_name, n_c):
#    "Fixes tables that were shifted into 1 row by inserting a space every n_c elements, to get n_c number of columns"
#    D=data_matrix(file_name)
#    outfile=open(file_name,"w")
#    print D
#    t=0
