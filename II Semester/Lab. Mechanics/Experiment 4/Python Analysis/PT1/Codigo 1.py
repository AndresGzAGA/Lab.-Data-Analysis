...
def err_a(o, do): #cm
    return np.sqrt(9780**2*(np.cos(2*o)**2)*do**2/4) #NOTA: Checar 9.78 en cm o m, o (theta) tiene que estar en radianes!!!
    
def err_b(o, do): #cm
    return np.sqrt(9780**2*(np.sin(2*o))**2*do**2/4)
...
def err_modely(x,b,dx,db):
    return np.sqrt(4*b**2*np.power(x,2)*dx**2+np.power(x,4)*db**2)
...
def fitx(file_name, Po, Po_err, ci=1, cf=1, xerr=2, yerr_type="Measurement", yerr=3):
    ...
    plt.fill_between(xdataE, modelx(xdataE,*Po)-err_modelx(xdataE,Po[0],0.016,err_a(o,do)), modelx(xdataE, *Po)+err_modelx(xdataE,Po[0],0.016,err_a(o,do)),alpha=0.5, edgecolor='#3F7F4C', facecolor='#7EFF99',linewidth=0)

    plt.xlabel('Tiempo [s]')
    plt.ylabel('x [mm]')
    plt.legend()
    plt.title('x(t) a 15 grados, vista no inclinada')
    plt.show()
    
    print popt
    print pcov

def fity(file_name, Po, Po_err, ci=1, cf=1, xerr=2, yerr_type="Measurement", yerr=3):
    """
    Fits the function func to the average of the data from ci to cf, 
    with the associated error for each row if err_type is "Measurement";
    if err_type is "Standard deviation", then it calculates the standard
    ...
    popt, pcov = curve_fit(modely, xdata, ydata, Po, sigma)
    plt.plot(xdataE, modely(xdataE, *popt), 'r-', label='fit')
    plt.fill_between(xdataE,modely(xdataE, *popt)-err_modely(xdataE,popt[0],0.016,pcov[0][0]),modely(xdataE, *popt)+err_modely(xdataE,popt[0],0.016,pcov[0][0]),alpha=0.5,edgecolor='#CC4F1B', facecolor='#FF9848')
    
    plt.plot(xdataE, modely(xdataE, *Po), 'g-', label='Model')
    plt.fill_between(xdataE, modely(xdataE,*Po)-err_modely(xdataE,Po[0],0.016,err_b(o,do)), modely(xdataE, *Po)+err_modely(xdataE,Po[0],0.016,err_b(o,do)),alpha=0.5, edgecolor='#3F7F4C', facecolor='#7EFF99',linewidth=0)

    plt.xlabel('Tiempo [s]')
    plt.ylabel('x [mm]')
    plt.legend()
    plt.title('y(t) a 15 grados, vista no inclinada')
    plt.show()
...
def P_file(file_name):
    ...
    for i in range(1,len(thetal)):
        a.append(-m.sin(2*thetal[i])*981/4)
    ...
    b=["b[cm/s^2]"]
    for i in range(1,len(thetal)):
        b.append(m.sin(thetal[i])**2*981/2)
        
    db=["db[cm/s^2]"]
    for i in range(1,len(a)):
        db.append(err_b(thetal[i],dol[i]))
    
    D=[]
    D.append([alphal[0],dal[0],thetal[0],dol[0],a[0],da[0],b[0],db[0]])
    for k in range(1,len(a)):
        row=[]
        row=[str(alphal[k]),str(dal[k]),str(round(thetal[k],3)),str(round(dol[k],4)),str(int(round(a[k]))),str(round(da[k],2)),str(int(round(b[k]))),str(round(db[k],2))]
        D.append(row)
