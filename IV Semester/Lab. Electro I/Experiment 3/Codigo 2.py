...
def model1(k, h):
    return np.sqrt(978*(k+h))

def err_model1(k, dk, h, dh):
    return 978/(2*model1(k,h))*np.sqrt(np.power(dk,2)+np.power(dh,2))
    
def model2(k, h):
    return np.sqrt(978*h)*(1+k/(2*h))

def err_model2(k, dk, h, dh):
    return np.sqrt(978/h)*np.sqrt(np.power(dk,2)+np.power((1-k/(2*h))*dh,2))/2
...
def fit(file_name, Po, Po_err, ci=1, cf=1, xerr=2, yerr_type="Measurement", yerr=3):
    """
    ...
    """
    dic_ydata=avrgc(file_name,ci,cf)
    ydata=[]
    ...
    sigma=[]
    if yerr_type=="Standard deviation":
        dic_sigma=stdc(file_name,ci,cf)
    ...
    n=20
    m=140 #Parameters for the lenght of the area of interest, n for adding length to the left, m to the right.
    E=(xdata[-1]-xdata[0])/len(xdata)
    #for i in range(1,n): #REMOVE THIS NEXT TIME
    xdataE.append(xdata[0]-n*E)
    for i in xdata:
        xdataE.append(i)
    for i in range(1,m):
        xdataE.append(xdata[-1]+i*E)
    
    xdataE=np.array(xdataE)
    
    #   2 MODELS
    
    popt, pcov = curve_fit(model1, xdata, ydata, Po, sigma)
    plt.plot(xdataE, model1(xdataE, *popt), 'r-', label='fit1')
    plt.fill_between(xdataE,model1(xdataE, *popt)-err_model1(xdataE,0,popt[0],pcov[0][0]),model1(xdataE, *popt)+err_model1(xdataE,0,popt[0],pcov[0][0]),alpha=0.5,edgecolor='#CC4F1B', facecolor='#FF9848')
    
    #Comparison to a model with theoretical parameters.
    plt.plot(xdataE, model1(xdataE, *Po), 'g-', label='Model1')
    plt.fill_between(xdataE, model1(xdataE,*Po)-err_model1(xdataE,0,Po[0],Po_err[0]), model1(xdataE, *Po)+err_model1(xdataE,0,Po[0],Po_err[0]),alpha=0.5, edgecolor='#3F7F4C', facecolor='#7EFF99',linewidth=0)
    
    print popt
    print pcov
    
    popt, pcov = curve_fit(model2, xdata, ydata, Po, sigma)
    plt.plot(xdataE, model2(xdataE, *popt), 'm-', label='fit2')
    plt.fill_between(xdataE,model2(xdataE, *popt)-err_model2(xdataE,0,popt[0],pcov[0][0]),model2(xdataE, *popt)+err_model2(xdataE,0,popt[0],pcov[0][0]),alpha=0.5,edgecolor='#FFCCFF', facecolor='#E5CCFF')
    
    #Comparison to a model with theoretical parameters.
    plt.plot(xdataE, model2(xdataE, *Po), 'c-', label='Model2')
    plt.fill_between(xdataE, model2(xdataE,*Po)-err_model2(xdataE,0,Po[0],Po_err[0]), model2(xdataE, *Po)+err_model2(xdataE,0,Po[0],Po_err[0]),alpha=0.5, edgecolor='#CCE5FF', facecolor='#CCFFFF',linewidth=0)
    ...
    plt.ylabel("c[cm/s]")
    plt.legend()
    plt.title('Velocidad en h4 con jabon')
    plt.show()
