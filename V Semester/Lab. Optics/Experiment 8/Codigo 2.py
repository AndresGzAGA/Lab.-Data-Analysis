...
def alpha(ti):
    return A/2+3*np.pi/2-np.radians(ti)

def delta(ti,tc):
    return np.radians(tc-ti)

def X(n,a):
    return np.sin(A)*np.sqrt((np.power(n,2)-np.power(np.sin(a),2)))-np.sin(a)*np.cos(A)

def model(a):
    n=1.5840
    return a+np.arcsin(np.sin(A)*np.sqrt((np.power(n,2)-np.power(np.sin(a),2)))-np.sin(a)*np.cos(A))-A

def err_model(a, da, n, dn):
    N=np.power(n,2)-np.power(np.sin(a),2)
    B=1-np.cos(a)*(np.sin(A)*np.sin(a)-np.cos(A)*np.sqrt(N))/np.sqrt(N*(1-np.power(np.sin(A)*np.sqrt(N)-np.sin(a)*np.cos(A),2)))
    C=n*np.sin(A)/np.sqrt(N*(1-np.power(np.sin(A)*np.sqrt(N)-np.sin(a)*np.cos(A),2)))
    return np.sqrt(np.power(B*da,2)+np.power(C*dn,2))
    
def n(L, P, Q):
    return P+Q/np.power(L,2)
    
def dn(L, dL, P, dP, Q, dQ):
    return np.sqrt(np.power(dP,2)+np.power(dQ/np.power(L,2),2)+np.power(2*Q*dL/np.power(L,3),2))
...
def fit(file_name, Po, Po_err, ci=2, cf=2, xerr=1, yerr_type="Measurement", yerr=3):
    ...
    d=200.0 #Parameter for density of points (bigger d, more points) MUST BE FLOAT
    ...
    plt.ylim(np.array(ydata).min()-0.1,np.array(ydata).max()+0.1)
    plt.locator_params(numticks=10)
    plt.xlabel(r"$\alpha$ [rad]")
    plt.ylabel(r"$\delta$ [rad]")
    plt.legend()
    plt.title(r'$\'Angulo\ de\ desviaci\'on\ total\ del\ L\'aser$')
    plt.show()
