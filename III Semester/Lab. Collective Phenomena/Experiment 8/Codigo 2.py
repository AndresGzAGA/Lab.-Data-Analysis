...
def model(F, c, n):
    return c*np.power(F,n)

def err_model(F, dF, c, dc, n, dn):
    return np.power(F,n)*np.sqrt(dc**2+(n*c)**2*np.power(F,-2)*np.power(dF,2)+np.power(c*np.log(F)*dn,2))
...
def fit(file_name, Po, Po_err, ci=1, cf=1, xerr=2, yerr_type="Measurement", yerr=3):
    ...
    plt.fill_between(xdataE,model(xdataE, *popt)-err_model(xdataE,3.7E-4,popt[0],pcov[0][0],popt[1],pcov[1][1]),model(xdataE, *popt)+err_model(xdataE,3.7E-4,popt[0],pcov[0][0],popt[1],pcov[1][1]),alpha=0.5,edgecolor='#CC4F1B', facecolor='#FF9848')
    
    #Not comparing to a model with theoretical parameters.
    plt.plot(xdataE, model(xdataE, *Po), 'g-', label='Model')
    plt.fill_between(xdataE, model(xdataE,*Po)-err_model(xdataE,3.7E-4,Po[0],Po_err[0],Po[1],Po_err[1]), model(xdataE, *Po)+err_model(xdataE,3.7E-4,Po[0],Po_err[0],Po[1],Po_err[1]),alpha=0.5, edgecolor='#3F7F4C', facecolor='#7EFF99',linewidth=0)
    
    plt.xscale("linear")
    plt.yscale("linear")
    plt.xlim(1E-4,1.8E-3)
    plt.ylim(60,150)
