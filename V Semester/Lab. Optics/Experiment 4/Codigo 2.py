...
def dist_plot(file_name,ci=2,cf=2,xerr=1,yerr_type="Measurement",yerr=3):
    ...
    x=avd.keys()
    x.sort()
    plt.plot(x, x, 'g-', label='Model')     #QUITAR
...
def model(O, a):
    O=np.radians(O)
    return np.degrees(np.arcsin(a*np.sin(O)))

def err_model(O, dO, a, da):
    O=np.radians(O)
    dO=np.radians(dO)
    return np.degrees(np.sqrt((np.power(dO*a*np.cos(O),2)+np.power(da*np.sin(O),2))/(1-np.power(a*np.sin(O),2))))
...
def fit(file_name, Po, Po_err, ci=2, cf=2, xerr=1, yerr_type="Measurement", yerr=3):
    ...
    n=4
    m=4 #Parameters for the lenght of the area of interest, n for adding length to the left, m to the right.
