...
def model(I, D):
    return L*Mo*np.power(I,2)/(2*np.pi*D)

def err_model(I, dI, D, dD):
    return Mo*I/(D*np.pi)*np.sqrt(np.power(I*dL/2,2)+np.power(L*dI,2)+np.power(L*I*dD/(2*D),2))
...
def fit(file_name, Po, Po_err, ci=1, cf=1, xerr=2, yerr_type="Measurement", yerr=3):
    ...
    n=10
    m=20 #Parameters for the lenght of the area of interest, n for adding length to the left, m to the right.
    ...
    plt.plot(xdataE, model(xdataE, *popt), 'g-', label='fit_rep')
