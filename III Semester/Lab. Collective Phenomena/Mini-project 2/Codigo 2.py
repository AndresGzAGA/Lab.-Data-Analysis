...
#def A(k,m,yo,c):
#    return 2*np.sqrt(k*m)*yo/np.sqrt(4*k*m-np.power(c,2))

#def r(m,c):
#    return c/(2*m)
    
#def f(k,m,c):
#    return np.sqrt(4*k*m-np.power(c,2))/(2*m)

#def p(k,m,c):
#    return np.arcsin(c/(2*np.sqrt(k*m)))

def model(t, a, b):
    return yo/(np.sqrt(1-a**2/b))*np.power(np.e,-a*(t-to))*np.cos(np.sqrt(b-a**2)*(t-to)-np.arcsin(a/np.sqrt(b)))

#def err_model(t, dt, A, dA, r, dr, f, df, p, dp):
#    return A*np.power(np.e,-r*(t-to))*np.sqrt(np.power(np.cos(f*(t-to)-p),2)*((dA/A)**2+np.power(t-to,2)*dr**2)+np.power(np.sin(f*(t-to)-p),2)*(np.power(t-to,2)*df**2+dp**2)+np.power(r*np.cos(f*(t-to)-p)+f*np.sin(f*(t-to)-p),2)*dt**2)
...
def fit(file_name, Po, Po_err, ci=1, cf=1, xerr=2, yerr_type="Measurement", yerr=3):
    ...
    #plt.fill_between(xdataE,model(xdataE, *popt)-err_model(xdataE,0.125,popt[0],pcov[0][0],popt[1],pcov[1][1],popt[2],pcov[2][2],popt[3],pcov[3][3]),model(xdataE, *popt)+err_model(xdataE,0.125,popt[0],pcov[0][0],popt[1],pcov[1][1],popt[2],pcov[2][2],popt[3],pcov[3][3]),alpha=0.5,edgecolor='#CC4F1B', facecolor='#FF9848')
    
    #Not comparing to a model with theoretical parameters.
    #plt.plot(xdataE, model(xdataE, *Po), 'g-', label='Model')
    #plt.fill_between(xdataE, model(xdataE,*Po)-err_model(xdataE,Po[0],0.016,Po_err[0]), model(xdataE, *Po)+err_model(xdataE,Po[0],0.016,Po_err[0]),alpha=0.5, edgecolor='#3F7F4C', facecolor='#7EFF99',linewidth=0)
