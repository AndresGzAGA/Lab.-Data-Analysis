...
b=3
c=2.5
charges = [LineCharge(-0.5, [-a, 0], [-c, 0]),
           LineCharge(-0.5,[-c,-b],[-c,b]),
           LineCharge(0.5, [a,0], [c, 0]),
           LineCharge(0.5,[c,-b],[c,b])]
field = ElectricField(charges)

# Create the flux points manually
fluxpoints = []
fluxpoints += list([-0.51, y] for y in numpy.linspace(-a, a, 15)[1:-1])
fluxpoints += list([-0.49, y] for y in numpy.linspace(-a, a, 15)[1:-1])
